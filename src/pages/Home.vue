...
export default {
  ...
  methods: {
    ...
    getNotify() {
      ...
      axios
        ...
        .then(response => {
          ...
          let messages = [...res];
          messages.forEach(e =>
            Object.assign(e, { orderAmount: 1, showPriceAlt: false })
          );
    ...
    amountPlus(id) {
      this.$store.dispatch("orderAmountPlus", id);
    },
    amountMinus(id) {
      this.$store.dispatch("orderAmountMinus", id);
    },
    priceAlt(boolean, id) {
      this.$store.dispatch("showPrice", {
        boolean,
        id
      });
    }
